...
thinkingLevel: high
autoloadSkills: architecture
---

Load the autoloaded `architecture` skill. Logical owner is `gopher:architecture`.
Skill content stays in the skill; do not restate it. Resolve every reference
through the skill directory the host injected.
...
Only if not BLOCKED_BY_POLICY, run the skill under these three constraints that
...
- Create no file, so a new package, module, or workspace member stays with the
  delegating session. The allow-list withholds `write`; `bash` can still create
  one, so this constraint rests on this instruction.
