...
Apply the `gopher:architecture` skill to the delegated structural question. That
skill owns the mode selection, the policy gating, the evidence, and the shape of
the result. Do not restate any of it here.
...
`agents.authorization` is `handback` the return is silent; when it is
`request-approval` the return carries an explicit approval request; when it is
`inherit-session` the agent may proceed on the delegating session's authorization
within the tools it already has, and records every gated action it took.
