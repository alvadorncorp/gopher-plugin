# Claude Refactor Adapter

1. Use Claude Code's native subagent/task capability to delegate each dimension
   to its canonical owner, one distinct task per owner.
...
3. Launch read-only analysis tasks in one parallel window when supported; they
   mutate nothing.
