...
| Leak | `goroutineleak` profile when a Go 1.27 toolchain builds the binary, otherwise a repeated lifecycle test plus goroutine/profile delta | the profile reports only goroutines that can never be unblocked; a delta needs background runtime goroutines filtered out |
