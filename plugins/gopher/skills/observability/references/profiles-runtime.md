...
exposure surface it creates, including the goroutine labels that reach
tracebacks from Go 1.27, the discovery rule for `runtime/metrics`, and the
...
| Which profile kinds are collected | Each sampled kind — CPU, heap, goroutine, mutex, block — carries a different overhead and a different enabling step; the Go 1.27 `goroutineleak` kind is computed during garbage collection instead |
...
A Go 1.27 toolchain adds a sixth kind, `goroutineleak`, served at
...
`gopher:concurrency` work; authorizing who may fetch it is a `gopher:security`
decision like any other pprof endpoint.
...
ordinary application logs. A GODEBUG default is selected by the module's `go`
directive, so this is one of the few Go 1.27 behaviors gated on the declared
version rather than on the building toolchain.
...
- Label keys and values fall under the redaction rules in
  `references/cardinality-cost-redaction.md`, not only the cardinality rules, and
  the absence assertion those rules require covers the traceback as well as the
  profile artifact.
- A project that keeps a label those rules forbid sets `tracebacklabels=0`
  deliberately and records why, rather than discovering the exposure from a log.
...
| Reading a `goroutineleak` profile to explain why a goroutine is stuck | `gopher:concurrency` |
