...
## `//line` directives are toolchain-sensitive
...
So the same generated bytes can produce different diagnostic paths before and
after the upgrade. When a reproduction differs only in reported paths, check the
toolchain row of the provenance record before treating it as a generator change,
and prefer emitting an absolute or repository-relative path the generator
computes itself over one that depends on where the compiler was invoked.
