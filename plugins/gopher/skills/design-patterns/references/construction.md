...
| `pattern.abstract-factory` | Diagnostic only: several real product families vary together. | Concrete constructors cover one family or speculative variants. | Type explosion and hidden coupling. | Demonstrate two active families and family-wide invariants. |
| `pattern.singleton` | Diagnostic only: process-wide identity is a verified invariant. | Explicit dependency wiring and scoped lifetime. | Global state, test interference, lifecycle ambiguity. | Prove unique identity is required rather than convenient. |
